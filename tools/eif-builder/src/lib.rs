...
// NOTE: The EIF header's architecture flag identifies the *target* arch of
// the kernel embedded in the EifSectionKernel section, not the arch of the
...
// target arch explicitly via `TargetArch`.
...
/// the upstream AWS Nitro Enclaves CLI. The rootfs is supplied out-of-band
/// as a virtio-blk device, so attestation of the rootfs cannot be derived
/// from the EIF alone. Consumers MUST NOT rely on EIF metadata for
/// attestation of artifacts produced by this builder.
