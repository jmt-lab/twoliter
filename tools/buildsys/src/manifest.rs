...
impl Manifest {
    ...
    pub fn guest_image_variant_deps(&self) -> Result<Vec<(String, PathBuf)>> {
        ...
        validate_guest_image_entries(host_name, guest_images)?;
...
/// self-reference, name well-formedness, path absoluteness, and path safety. Caller is
/// responsible for the graph-level "must be a direct build-dep" check.
///
...
"#;
    ...
    fn test_validate_guest_image_entries_accepts_well_formed_input() {
        ...
        validate_guest_image_entries("host-variant", &map).expect("well-formed input must pass");
    }
    ...
    fn test_validate_guest_image_entries_rejects_self_reference() {
        ...
        let err = validate_guest_image_entries("host-variant", &map)
    ...
    fn test_validate_guest_image_entries_rejects_invalid_names() {
        for bad_name in [":sneaky", "with:colon", "new\nline", ""] {
            ...
            let err = validate_guest_image_entries("host", &map).expect_err(&format!(
    ...
    fn test_validate_guest_image_entries_rejects_invalid_paths() {
        ...
        let err = validate_guest_image_entries("host", &map)
        ...
        assert!(
            validate_guest_image_entries("host", &map).is_err(),
        ...
        let err =
            validate_guest_image_entries("host", &map).expect_err("'..' in path must be rejected");
        let msg = format!("{err}");
        assert!(
