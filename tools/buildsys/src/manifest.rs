...
impl ManifestInfo {
    ...
    /// otherwise. Returned as-is (a `u16`); the value written into the EIF
    /// header is `arch_flags | eif_pcie_flags`, matching what
    /// `eif-builder --pcie-flags <hex>` does internally.
    ///
    /// See the `eif_pcie_flags` field on [`BuildVariant`] for the
    /// header==launch-flags contract this knob exists to author.
    ...
    /// x86_64; ignored otherwise. Returns `None` when unset, in which case
    /// `rpm2eif` applies its built-in default (bzImage — the sidecar
    /// enclave loader's boot protocol).
    ///
    /// See the `eif_kernel_format` field on [`BuildVariant`] for why this
    /// stays selectable.
