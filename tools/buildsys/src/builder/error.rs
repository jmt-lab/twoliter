...
pub(crate) enum Error {
    ...
    #[snafu(display(
        "EIF signing cert '{}' does not exist or is empty (from Infra.toml [eif].signing_cert). \
         Note: an empty file is rejected because the in-container helper's `[[ -s ]]` gate would \
         silently treat it as \"no profile\" and produce an unsigned EIF.",
        path.display()
    ))]
    ...
    #[snafu(display(
        "EIF signing key '{}' does not exist or is empty (from Infra.toml [eif].signing_key). \
         Note: an empty file is rejected because the in-container helper's `[[ -s ]]` gate would \
         silently fall back to \"no profile\" and produce an unsigned EIF.",
        path.display()
    ))]
