...
/// variant (or bring one into the local test project).
#[tokio::test]
#[ignore]
async fn test_twoliter_repack_variant_eif() {
    ...
    // The repacked EIF must exist under the expected build dir. Assert on
    // real regular files, not on directory presence, so a regression that
    // makes `eif2eif` silently produce no artifacts fails this test.
    let out_dir = bob_src
        .path()
        .join("build")
        .join("images")
...
///
/// This exercises the `img2img --guest-images=...` path added by this change:
...
async fn test_twoliter_repack_variant_resigns_guest_eifs() {
    ...
    // pointing at an EIF guest. Adjust to whichever upstream host+guest pair
    // currently ships in bottlerocket-os.
    ...
    // Snapshot the SHA-256 of the host's `.img.lz4` output before repack.
    // The host embeds guest EIFs inside its ROOT-A; when img2img resigns
    // any of them, the outer compressed image bytes will differ. This is
    // a weaker signal than extracting the guest EIF and comparing its
    // signature section directly (which would require unpacking the host
    // rootfs — non-trivial), but it does distinguish "repack ran and
    // touched the rootfs" from "repack was a no-op".
